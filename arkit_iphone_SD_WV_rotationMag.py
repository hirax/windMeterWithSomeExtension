...
if __name__ == '__main__':
    ...
    def measureWind():
        data = []; isSeek = True; distance = None
        timelimit = 50 # 無風に近く、風速計が回らない場合のリミット
        while(isSeek):
            data.append( motion.get_magnetic_field() )
            if( len(data)>1 ):       # 前回との極性比較ができる状態になったら
                if distance is None:   # 極性変化をまだ検知していなかったら
                    if np.sign(data[-2])[0] != np.sign(data[-1])[0]:
                        distance = 1;  # 極性変化が生じたので、周期計測開始
                else:                  # 前回極性変化からの周期計測を行う
                    if np.sign(data[-2])[0] != np.sign(data[-1])[0]:
                        isSeek = False # 周期計測終了
                    else:
                        distance = distance +1
            timelimit = timelimit - 1  # タイムリミットまでカウントダウン
            if timelimit == 0:         # タイムリミットなら強制終了
                distance = timelimit; isSeek = False
            time.sleep(0.01)
        return timelimit - distance
    
    def createNodeElementInfo():
        wv = measureWind()
        return 'sphere', [0.5+wv/50, 0.5, 0.5-wv/55, 0.6], 0.005, None, gRotation
